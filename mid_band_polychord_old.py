...
def measured_data(vlow, vhigh):

	tx = np.genfromtxt('/home/raul/DATA/EDGES/mid_band/spectra/level5/case2/case2_2hr_average_temperature.txt')
	wx = np.genfromtxt('/home/raul/DATA/EDGES/mid_band/spectra/level5/case2/case2_2hr_average_weights.txt')
	vx = np.genfromtxt('/home/raul/DATA/EDGES/mid_band/spectra/level5/case2/case2_2hr_average_frequency.txt')



	index = 6  # GHA=12-14 hr
	ty = tx[index,:]
	wy = wx[index,:]


	# Select frequency range
	vk = vx[(vx >= vlow) & (vx <= vhigh)]
	tk = ty[(vx >= vlow) & (vx <= vhigh)]
	wk = wy[(vx >= vlow) & (vx <= vhigh)]


	# Keep only data points with weight > 0
	v = vk[wk>0]
	t = tk[wk>0]
	w = wk[wk>0]
	...
	std_noise     = 0.020   # K
	sigma         = np.diag((std_noise**2)*np.ones(len(v)))     # uncertainty covariance matrix
	inv_sigma     = sp.linalg.inv(sigma)
	det_sigma     = np.linalg.det(sigma)



	return v, t, sigma, inv_sigma, det_sigma
...
v, t, sigma, inv_sigma, det_sigma = measured_data(61, 120)
...
def prior(cube):
	...
	T21_min = -1  # lower bound on uniform prior 
	T21_max =  1  # upper bound on uniform prior 

	vr_min =   61  # lower bound on uniform prior 
	vr_max =  100  # upper bound on uniform prior 

	dv_min =    2  # lower bound on uniform prior 
	dv_max =   30  # upper bound on uniform prior 

	tau_min =   0  # lower bound on uniform prior 
	tau_max =  20  # upper bound on uniform prior 


	a0_min =  738  # lower bound on uniform prior 
	a0_max =  740  # upper bound on uniform prior 	

	a1_min = -1e2  # lower bound on uniform prior 
	a1_max =  1e2  # upper bound on uniform prior 

	a2_min = -1e2  # lower bound on uniform prior 
	a2_max =  1e2  # upper bound on uniform prior 

	a3_min = -1e2  # lower bound on uniform prior 
	a3_max =  1e2  # upper bound on uniform prior 

	a4_min = -1e2  # lower bound on uniform prior 
	a4_max =  1e2  # upper bound on uniform prior 	
...
def triangle_plot():
	...
	d = np.genfromtxt('/home/raul/Desktop/test.txt')
