...
class TestFlagDataOutsideTemperatureRange:
            ...
            QTable(
                {
                    ...
                    'load_resistance': np.random.normal(loc=18000, scale=100, size=100) * un.Ohm
                }
            )
        )
    ...
    def test_all_flagged(self):        
        with pytest.raises(RuntimeError, match='The temperature range has masked all spectra'):
