import numpy as np
from edges.cal import Calibrator, CalibrationObservation
...
def test_cal_uncal_round_trip(calobs: CalibrationObservation, calibrator: Calibrator):
    a, b = calibrator.get_linear_coefficients(freqs=calobs.freqs, ant_s11=calobs.open.s11.s11)
    ...
    decal = calibrator.decalibrate(
        tcal, ant_s11=calobs.open.s11.s11,
    )
    np.testing.assert_allclose(
        decal, calobs.open.averaged_Q, atol=3e-5
    )
    
    new_tcal = calibrator.calibrate_q(
        decal, ant_s11=calobs.open.s11.s11,
    )
    
    np.testing.assert_allclose(
        new_tcal, tcal, atol=1e-6
    )
