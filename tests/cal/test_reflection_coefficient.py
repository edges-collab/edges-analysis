...
class TestTwoPortNetwork:
    ...
    def test_roundtrip_abcd(self):
        abcd = np.random.normal(size=(2, 2, 1))
    ...
    @pytest.mark.parametrize("addfunc", ["add_in_series", "add_in_parallel", "add_in_series_parallel", "cascade_with"])
    def add_bad(self, addfunc):
        ...
        with pytest.raises(ValueError, match='Two matrices must have the same dimensions'):
    ...
    def test_add_in_series_parallel(self):
        network1 = rc.TwoPortNetwork([[1, 0], [0, 1]])
        network2 = rc.TwoPortNetwork([[1, 0], [0, 1 + 0.1j]])

        combined = network1.add_in_series_parallel(network2)
        not_expected = rc.TwoPortNetwork([[2, 0], [0, 2 + 0.1j]])
...
class TestSMatrix:
    ...
    def test_properties(self):
        ...
        assert np.allclose(self.smatrix.scalar_logarithmic_gain, 20 * np.log10(np.abs(self.smatrix.s21)))
        assert np.allclose(self.smatrix.insertion_loss, -self.smatrix.scalar_logarithmic_gain)
        assert np.allclose(self.smatrix.input_return_loss, -20 * np.log10(np.abs(self.smatrix.s11)))
        assert np.allclose(self.smatrix.output_return_loss, -20 * np.log10(np.abs(self.smatrix.s22)))
        assert np.allclose(self.smatrix.reverse_gain, 20 * np.log10(np.abs(self.smatrix.s12)))
