...
from edges.cal import CalibrationObservation, Calibrator, CalibratedS11
...
class TestCalibrationObservation:
    ...
    def test_calibrator_resids(
        self, 
        ideal_calobs: CalibrationObservation, 
        identity_calibrator: Calibrator
    ):
        out = ideal_calobs.get_calibration_residuals(identity_calibrator)
        
        ld = ideal_calobs.loads
        cmp = {k: ideal_calobs.loads[k].averaged_Q - ld[k].temp_ave.to_value("K") for k, v in out.items()}
    ...
    def test_rms(self, ideal_calobs: CalibrationObservation, identity_calibrator: Calibrator):
    ...
    def test_update(self, calobs: CalibrationObservation):
        c2 = calobs.clone(
            receiver=CalibratedS11(
                s11=np.zeros(calobs.freqs.size, dtype=complex), 
                freqs=calobs.freqs
            )
        )
        assert c2 != calobs
    ...
    def test_calobs_equivalence(self, calobs: CalibrationObservation, caldef: CalObsDefEDGES2):
