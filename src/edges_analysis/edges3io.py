...
def get_s1p_files(load, year, day, root_dir="/data5/edges/data/EDGES3_data/MRO/"):
    ...
    if len(file_temp) == 0:
        raise OSError("No files found in {root_dir} for {year}, {day}, {load}")
    elif len(file_temp) > 1:
        raise OSError("More than one file found for {year}, {day}, {load}")
...
def extract_temperature(
    ...
    front_end_temp = df[(df["Time"] >= start_time) & (df["Time"] <= end_time)][
        "Front End temperature"
    ]
    amb_load_temp = df[(df["Time"] >= start_time) & (df["Time"] <= end_time)][
        "Amb load temperature"
    ]
    hot_load_temp = df[(df["Time"] >= start_time) & (df["Time"] <= end_time)][
        "Hot load temperature"
    ]
    inner_box_temp = df[(df["Time"] >= start_time) & (df["Time"] <= end_time)][
        "Inner box temperature"
    ]
    therm_control = df[(df["Time"] >= start_time) & (df["Time"] <= end_time)][
        "Thermal Control"
    ]
    battery_voltage = df[(df["Time"] >= start_time) & (df["Time"] <= end_time)][
        "Battery Voltage"
    ]
    battery_current = df[(df["Time"] >= start_time) & (df["Time"] <= end_time)][
        "PR59 Current"
    ]
