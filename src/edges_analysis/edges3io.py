...
def get_s1p_files(load,year, day):
    ...
    file_temp = glob(root_dir+'/'+year+'_'+day+'*'+'[00-99]_'+load+'.s1p')
    ...
    for name, label in {'open':'O', 'short':'S','match':'L'}.items():
            ...
            if load == 'lna':
                ...
                file_temp = glob(root_dir+'/'+year+'_'+day+'*'+'[00-99]_lna_'+label+'.s1p')
                assert len(file_temp)==1
                file_list[name] = file_temp[0]
                
                    
            else:
                file_temp = glob(root_dir+'/'+year+'_'+day+'*'+'[00-99]_'+label+'.s1p')
                assert len(file_temp)==1
                file_list[name] = file_temp[0]
...
def agilent(freq, res, delayps):
    ...
    loss = 2.30*1e9
#    print("loss %f Gohm/s\n",loss/1e9);
    Zcab = 50 + (1 - 1j)*(loss/(2*2*np.pi*freq*1e6))*np.sqrt(freq*1e6/1e9)
#    delay = 33e-12;
    delay = delayps*1e-12
    rload = res
#    print("Zcab %f %f\n",creal(Zcab),cimag(Zcab));
    gl = loss*(delay/(2*50))*np.sqrt(freq*1e6/1e9) + 1j*(2*np.pi*freq*1e6*delay + (loss*delay/(2*50))*np.sqrt(freq*1e6/1e9))
...
def cabl(freq, delay, Tin, lossf,dielf):
    ...
    d2 = np.sqrt(1.0/(np.pi*4.0*np.pi*1e-7*5.96e07*0.8*lossf)) #   // for tinned copper
    d = np.sqrt(1.0/(np.pi*4.0*np.pi*1e-7*5.96e07*lossf)) #  // skin depth at 1 Hz for copper
...
def gets11(load, year, day, cablen = 0, cabloss = 0, cabdiel = 0):
    ...
    res ={'open':1e9, 'short':0, 'load':49.962} #resistance of the calibration modes
    ...
    s11, s1221, s22 = cabsparams(t_gamma_open, t_gamma_short, t_gamma_load, vna_open.s11, vna_short.s11, vna_load.s11)
    ...
    if(load=='lna'):
        ...
        T, s11, s12 = cabl(vna_input.freq.freq.value, np.abs(cablen)*2.54e-2/3e08, 0, 1+cabloss*0.01 , 1+cabdiel*0.01)
