...
def level3_single_file_test(path_file, GHA_1, GHA_2, FLOW, FHIGH, plot_residuals_yes_no, model_type,
                            Nfg, save_yes_no, save_spectrum_name):
    ...
    if GHA_2 > GHA_1:
        avr, avw = tools.spectral_averaging(r[(gha >= GHA_1) & (gha <= GHA_2), :],
                                         w[(gha >= GHA_1) & (gha <= GHA_2), :])
...
def data_selection(m, GHA_or_LST='GHA', TIME_1=0, TIME_2=24, sun_el_max=90, moon_el_max=90,
                   amb_hum_max=200, min_receiver_temp=0, max_receiver_temp=100):
    # Master index
    ...
    index_Trec = index[(m[:, 11] >= min_receiver_temp) & (m[:,
                                                          11] <= max_receiver_temp)]  #
    # Combined index
    if TIME_1 < TIME_2:
...
def level4_save_averaged_spectra(case, GHA_case, first_day, last_day):
    if case == 2:
        ...
        header_text = 'f [MHz], t_ant (GHA=0-23) [K], std (GHA=0-23) [K], Nsamples (GHA=0-23)'
        file_path = edges_folder + \
                    'mid_band/spectra/level4/case_nominal_50' \
        ...
        header_text = 'f [MHz], t_ant (GHA=0-23) [K], std (GHA=0-23) [K], Nsamples (GHA=0-23)'
        file_path = edges_folder + \
                    'mid_band/spectra/level4/case_nominal_14_14_terms_55' \
    ...
    if GHA_case == 24:
        for i in range(24):
            fb, tb, rb, wb, sb = tools.level4_integration(case, [i], first_day, last_day, 55, 150, 5)
            if start == 0:
