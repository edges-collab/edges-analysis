...
from .alanmode import Edges2CalobsParams, Edges3CalobsParams, ACQPlot7aMoonParams, EdgesScriptParams
...
app.command(
    amode := App(
        name="alanmode", 
        help="Run scripts similar to legacy C code")
)
...
def _alancal(
    ...
    write_s11_csv(calobs._raw_receiver.freqs, calobs._raw_receiver.s11, out / "s11lna.csv")
    
    _make_plots(out, calobs, calibrator, load_s11_mdl, rcv_s11_mdl, hot_loss_model, opts.plot, t_load=opts.avg.tload, t_load_ns=opts.avg.tcal)


@amode.command(name='cal-edges2')
def alancal_edges2(
    data: Edges2CalobsParams,
    opts: Annotated[AlanCalOpts, Parameter(name='*')]
):
