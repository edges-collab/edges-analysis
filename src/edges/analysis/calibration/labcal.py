...
from ...cal import CalibrationObservation, Calibrator, s11
from ...cal import noise_waves as nw
...
class LabCalibration:
    ...
    calobs: Calibrator = attrs.field(
        converter=lambda x: (
            x.to_calibrator() if isinstance(x, CalibrationObservation) else x
        )
    )
    _antenna_s11_model: s11.AntennaS11 | Callable = attrs.field()
    ...
    def from_s11_files(
        ...
        **kwargs,
        ...
        if hasattr(s11_files, "__len__") and len(s11_files) == 4:
            ant_s11 = s11.AntennaS11.from_s1p_files(
                files=s11_files,
            ...
            ant_s11 = s11.AntennaS11.from_single_file(
                s11_files,
                f_low=calobs.freq.min(),
                f_high=calobs.freq.max(),
                internal_switch=calobs.internal_switch,
    ...
    def antenna_s11_model(self) -> Callable[[np.ndarray], np.ndarray]:
        ...
        return (
            ...
            if isinstance(self._antenna_s11_model, s11.AntennaS11)
    ...
    def antenna_s11(self) -> np.ndarray:
        ...
        return self.antenna_s11_model(self.calobs.freq.to_value("MHz"))
