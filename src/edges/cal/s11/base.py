...
class CalibratedS11:
    ...
    s11: np.ndarray = npfield(
        dtype=complex, possible_ndims=(1,),
    )
    ...
    def _fv(self, att, val):
        if val.size != len(self.s11):
            raise ValueError(
                f"len(freq) != len(raw_s11) [{len(val)},{len(self.s11)}]"
            )

    # Constructor Methods
    @classmethod
    def from_receiver_filespec(cls, pathspec: calobsdef.ReceiverS11 | Sequence[calobsdef.ReceiverS11], **kwargs) -> Self:
    ...
    def from_load_and_switch(
        cls, loaddef: calobsdef.LoadS11, switchdef: calobsdef.SwitchingState | None, 
        **kwargs) -> Self:
        from .cal_loads import get_loads11_from_load_and_switch
        return get_loads11_from_load_and_switch(loaddef=loaddef, switchdef=switchdef, **kwargs)

    @classmethod
    def from_edges2_loaddef(cls, caldef: calobsdef.CalObsDefEDGES2, load: str, **kwargs) -> Self:
        from .cal_loads import get_loads11_from_edges2_loaddef
        return get_loads11_from_edges2_loaddef(caldef, load, **kwargs)

    @classmethod
    def from_edges3_loaddef(cls, caldef: CalObsDefEDGES3, load: str, calkit: rc.Calkit = rc.AGILENT_ALAN, **kwargs) -> Self:
        from .cal_loads import get_loads11_from_edges3_loaddef
        return get_loads11_from_edges3_loaddef(caldef, load=load, calkit=calkit, **kwargs)
...
class CalibratedSParams:
    ...
    def smoothed(
        ...
        freqs
            ...
            as in this object (i.e. only smoothing, no interpolation).
        """
        from .s11model import new_s11_modelled, S11ModelParams

        if isinstance(params, S11ModelParams):
            params = {'s11': params, 's12': params, 's21': params, 's22': params}
            
        s11 = new_s11_modelled(self.s11, params['s11'], freqs=self.freqs, new_freqs=freqs)
        s12 = new_s11_modelled(self.s12, params['s12'], freqs=self.freqs, new_freqs=freqs)
        s21 = new_s11_modelled(self.s21, params.get('s21', params['s12']), freqs=self.freqs, new_freqs=freqs)
        s22 = new_s11_modelled(self.s22, params['s22'], freqs=self.freqs, new_freqs=freqs)
